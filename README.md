...
VaultPilot 1.1.0 requires Obsidian 1.11.4 or newer so Gemini credentials can use Obsidian SecretStorage.
...
On-screen Enter inserts a newline on mobile; tap **Send** to submit. A connected hardware keyboard can submit with Ctrl+Enter or Cmd+Enter. Session cost remains in the chat header because Obsidian mobile has no bottom status bar.
...
Vault notes selected for chat context, memory processing, or embedding generation are sent directly from Obsidian to Google's Gemini API. Embedding vectors and their source chunks are stored locally in the browser profile's IndexedDB rather than as JSON files in the vault. DuckDuckGo queries and returned public pages pass directly between Obsidian and DuckDuckGo.
...
- **Network use:** Prompts, selected note context, memory-extraction inputs, and embedding chunks are sent directly to Google Gemini when their corresponding features run. Web-search queries are sent directly to DuckDuckGo. No VaultPilot-operated server exists.
...
src/
  ...
  storage/                IndexedDB vectors and short-term chat sessions
...
See `outputs/ARCHITECTURE.md` for the detailed system specification and runtime flows.
